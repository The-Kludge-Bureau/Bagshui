    ...
    for recid, recdata in pairs(data) do
      for id, num in pairs(recdata.ing) do
        if id == itemid then
          ...
          retval[recid] = self.compost and self.compost:Acquire(name, recdata.skill, num)
            or { name, recdata.skill, num }
        end
      end
    end
  end
...
function lib:TradeUsesItem(trade, itemid)
  if not self.tradedata[trade] then
    return
  end
  ...
  for i = 1, GetNumTradeSkills() do
    ...
    if t ~= "header" then
      ...
      if link then
        ...
        self.tradedata[trade][tonumber(id)] = self.tradedata[trade][tonumber(id)]
          or { ing = self:ParseTradeskillItem(i), skill = self.quals[t] }
      end
    end
  end
end
  ...
  for i = 1, GetNumCrafts() do
    ...
    if t ~= "header" then
      ...
      if link then
        ...
        self.tradedata[trade][tonumber(id)] = self.tradedata[trade][tonumber(id)]
          or { ing = self:ParseCraftItem(i), skill = self.quals[t], name = name }
      end
    end
  end
end
