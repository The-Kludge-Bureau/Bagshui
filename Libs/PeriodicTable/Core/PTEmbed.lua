...
if libobj and not libobj:NeedsUpgraded(vmajor, vminor) then
  return
end
...
if not stubobj then
  ...
  function stubobj:NeedsUpgraded(vmajor, vminor)
    ...
    if not versionData or versionData.minor < vminor then
      return true
    end
  end
  ...
  function stubobj:GetInstance(version)
    if not version then
      version = self.lastVersion
    end
    local versionData = self.versions[version]
    if not versionData then
      print(string.format("<%s> Cannot find library version: %s", self.libName, version or ""))
      return
    end
...
function lib:CacheSet(set)
  ...
  if not rset or type(rset) ~= "string" then
    return
  end

  if not self.vars.cache then
    self.vars.cache = {}
  end
...
function lib:CreateTrashTable(set, bosses)
  ...
  for i, v in pairs(t) do
    if not self:ItemInSet(i, bosses) then
      retval[i] = v
    end
  end
...
function lib:AddModule(name, table, memory)
  if not name or not table or not memory then
    return
  end
  if self.k[name] and self.compost then
    self.compost:Reclaim(self.k[name])
  end
  self.k[name] = table
...
function lib:ItemInSets(item, sets)
  ...
  if type(sets) == "string" then
    ...
    if type(rset) == "string" then
      ...
      if inset then
        return self.compost and self.compost:Acquire(sets) or { sets }
      end
    elseif type(rset) == "table" then
  ...
  elseif type(sets) == "table" then
    ...
    for i, s in pairs(sets) do
      if self:ItemInSet(item, s) then
        if not founds then
          founds = self.compost and self.compost:Acquire() or {}
        end
...
local defaultcomparefunc = function(abag, aslot, aval, bbag, bslot, bval)
  return aval > bval
end
local defaultvalidatefunc = function(bag, slot, val)
  return true
end
...
function lib:GetSetTable(set)
  if self:CacheSet(set) then
    return self.vars.cache[set]
  end
end
...
function lib:RegisterCustomSet(setcode, name)
  ...
  if self:GetSetModule(setname) then
    return
  end
  ...
  if not name then
    self.vars.numcustoms = self.vars.numcustoms + 1
  end
  return setname
end
