...
Bagshui:AddComponent(function()
  ...
  function CharacterData:Init()
    ...
    self.idListSortFunctions.realm = function(idA, idB)
      return (self._idListSortFunctions_objectList[idA].realm or "")
        < (self._idListSortFunctions_objectList[idB].realm or "")
    end
    self.sortedIdLists.lastLogout = {}
    self.idListSortFunctions.lastLogout = function(idA, idB)
      return (self._idListSortFunctions_objectList[idA].lastLogout or 0)
        < (self._idListSortFunctions_objectList[idB].lastLogout or 0)
    end
    ...
    Bagshui.prototypes.Menus:AddAutoSplitMenu(BS_AUTO_SPLIT_MENU_TYPE.CHARACTERS, {
      ...
      -- Not much to configure since there's no editing.
      {
        ...
        managerColumnTextFunc = function(fieldName, id, obj, preliminaryDisplayValue)
          ...
          elseif fieldName == "lastLogout" then
            return _G.date(
              "%Y-%b-%d",
              type(preliminaryDisplayValue) == "number" and preliminaryDisplayValue or _G.time()
            )
          end
