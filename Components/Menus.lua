...
Bagshui:AddComponent(function()
  ...
  function Menus:Init()
    ...
    self.autoSplitMenuRefreshHackText = self.autoSplitMenuRefreshHackFrame:CreateFontString(
      autoSplitMenuRefreshHackFrameName .. "Text",
      nil,
      "GameFontNormal"
    )
  ...
  function Menus:OpenMenuCallback()
    ...
    if
      ...
      and openMenuFunc(self.menuList[self._openMenu_MenuType], self._openMenu_Arg1, self._openMenu_Arg2)
        == false
    then
      doShowMenu = false
    end
  ...
  function Menus:ShowMenu(menuType, anchorFrame, xOffset, yOffset, anchorPoint, anchorToPoint)
    ...
    -- Open the menu.
    _G.ToggleDropDownMenu(1, nil, Bagshui.menuFrame, anchorFrame, xOffset, yOffset)
  ...
  function Menus:LoadMenu(menuType, level)
    ...
    if
      level == 1
      and self.menuList[menuType]
      and self.menuList[menuType].levels[1]
      and self.menuList[menuType].levels[1].autoSplitMenuType
    then
    ...
    if not menuItems then
      Bagshui:PrintError(
        "Menu not found for type " .. menuType .. " level " .. tostring(menuLevel) .. " value " .. tostring(menuValue)
      )
      return
    end
  ...
  function Menus:PrepareAutoSplitMenu(menuValue)
    ...
    assert(
      self.autoSplitMenuList[autoSplitMenuType],
      "Menus:PrepareAutoSplitMenu(): Auto-Split menu type '" .. tostring(autoSplitMenuType) .. "' not found"
    )
    ...
    local numExtraItems = (type(baseExtraMenuitems) == "table" and table.getn(baseExtraMenuitems) or 0)
    ...
    for _, id in ipairs(idList) do
      if
        ...
        and ((type(omitFunc) ~= "function" or type(itemList) ~= "table") or not omitFunc(id, itemList))
      then
    ...
    if submenuCount > 1 then
      ...
      for submenuNum = 1, submenuCount do
        ...
        menuStartText, menuEndText =
          self:BuildAutoSplitSubmenuRangeTexts(submenuFirstItem, submenuLastItem, prevNextMenuStartText, nil)
        ...
        nextSubmenuFirstItem = (lastIndex + 1 <= idCount)
        ...
        nextMenuStartText = (nextSubmenuFirstItem ~= nil)
            and string.sub(nextSubmenuFirstItem, 1, string.len(menuEndText))
          or ""
        ...
        if nextMenuStartTextMinLength > 0 then
          menuEndText, nextMenuStartText =
            self:BuildAutoSplitSubmenuRangeTexts(submenuLastItem, nextSubmenuFirstItem, nil, nextMenuStartTextMinLength)
        else
  ...
  function Menus:PopulateAutoSplitMenuItems(
    ...
    for i = firstIndex or 1, lastIndex or table.getn(idList) do
      ...
      if type(idList[i]) == "table" then
        ...
        menuItem.checked = BsUtil.TableContainsValue(idsToCheck, itemId) ~= nil
          or (type(checkFunc) == "function" and checkFunc(itemId, menuItem, triggeringMenuValue))
  ...
  function Menus:BuildAutoSplitSubmenuRangeTexts(startItemName, endItemName, previousStartText, textLengthStart)
    ...
    repeat
      ...
      startText = (previousStartText ~= nil and BsUtil.Utf8Len(previousStartText) >= submenuTextCharCount)
      ...
      if startText ~= endText then
        ...
        local startWithoutTrailingPunctuation = not string.find(startText, "^%p+$")
            and string.gsub(startText, "%W+$", "")
          or startText
        local endWithoutTrailingPunctuation = not string.find(endText, "^%p+$") and string.gsub(endText, "%W+$", "")
          or endText
    ...
    until (
      ...
      or (startText == startItemName and endText == endItemName)
