...
Bagshui:LoadComponent(function()
  ...
  function Bagshui:ToggleDropDownMenu(
    ...
    if level == 1 then
      ...
      then
        ...
        if
          self.menuFrame.bagshuiData.anchorPoint ~= nil
          and point ~= nil
          and point ~= self.menuFrame.bagshuiData.anchorPoint
        then
          ...
          elseif string.find(self.menuFrame.bagshuiData.anchorPoint, "RIGHT$") and string.find(point, "LEFT$") then
        ...
        if correctedAnchorPoint and correctedAnchorToPoint then
          ...
          frame:SetPoint(
            correctedAnchorPoint,
            (self.menuFrame.bagshuiData.anchorToFrame or anchorName),
            correctedAnchorToPoint,
            xOffset,
            yOffset
          )
      ...
      self._toggleDropDownMenu_ExpandMenusLeft = (
        frame:GetLeft()
        and ((frame:GetRight() * frame:GetScale()) > (_G.UIParent:GetRight() * _G.UIParent:GetScale()) / 2)
      )
    end
    ...
    if
      phase < 2
      and level > 1 -- Submenu
      and (adjustX < 0 or self._toggleDropDownMenu_ExpandMenusLeft)
    then
      ...
      if frame:GetLeft() * frame:GetScale() > parentFrame:GetLeft() * parentFrame:GetScale() then
        ...
        fullMoveLeft = -((parentFrame:GetWidth() * parentFrame:GetScale()) + (frame:GetWidth() * frame:GetScale()))
          + BsSkin.menuShiftLeft
    ...
    if level > 1 and adjustX == 0 and BsSkin.menuShiftRight ~= 0 then
  ...
  function Bagshui:UIDropDownMenu_AddButton(wowApiFunctionName, menuItem, level)
    ...
    if expandArrow then
      ...
      xOffset = (menuItem._bagshui and menuItem.hasArrow and menuItem.notCheckable) and -10
        or self._originalExpandArrowXOffset
    ...
    if menuItem.icon and icon then
      ...
      local a = (menuItem._bagshui and menuItem.disabled and not menuItem.isTitle) and 0.5
        or menuItem._bagshuiTextureA
        or 1
      ...
      -- Icon position.
      xOffset = menuItem._bagshuiTextureXOffset
        or (menuItem._bagshui and ((menuItem.hasArrow and not menuItem._bagshuiHideArrow) and (menuItem.notCheckable and -28 or -24) or menuItem._bagshuiFakeTitle and -14 or menuItem.notCheckable and -16) or -12)
